...
describe("DoctorService", () => {
  ...
  type ServiceRepositories = {
    ...
    specialtyRepository: jest.Mocked<SpecialtyRepository>;
  };
  ...
  beforeEach(() => {
    ...
    mockSpecialtyRepo = repositories.specialtyRepository;
  });

  // ─── findAll ──────────────────────────────────────────────
  describe("findAll", () => {
    it("deve retornar lista de médicos", async () => {
      mockDoctorRepo.findAll.mockResolvedValue([mockDoctor as Doctor]);
  ...
  describe("create", () => {
    ...
    it("deve criar um médico com sucesso", async () => {
      ...
      mockSpecialtyRepo.findById.mockResolvedValue(mockSpecialty as Specialty);
      const createdDoctor = { ...mockDoctor, ...createDTO };
      mockDoctorRepo.create.mockResolvedValue(createdDoctor as Doctor);
      mockDoctorRepo.findById.mockResolvedValue(createdDoctor as Doctor);
    ...
    it("deve lançar AppError 409 quando CRM já existe", async () => {
      mockDoctorRepo.findByCrm.mockResolvedValue(mockDoctor as Doctor);
    ...
    it("deve lançar AppError 409 quando email já existe", async () => {
      mockDoctorRepo.findByCrm.mockResolvedValue(null);
      mockDoctorRepo.findByEmail.mockResolvedValue(mockDoctor as Doctor);
      await expect(service.create(createDTO)).rejects.toMatchObject({
        statusCode: 409,
  ...
  describe("update", () => {
    it("deve atualizar o médico com sucesso", async () => {
      ...
      mockDoctorRepo.findById.mockResolvedValue(mockDoctor as Doctor);
      mockDoctorRepo.update.mockResolvedValue(updatedDoctor as Doctor);
    ...
    it("deve lançar AppError 409 quando novo email já pertence a outro médico", async () => {
      ...
      mockDoctorRepo.findById.mockResolvedValue(mockDoctor as Doctor);
      mockDoctorRepo.findByEmail.mockResolvedValue(outroDoctor as Doctor);
      await expect(
